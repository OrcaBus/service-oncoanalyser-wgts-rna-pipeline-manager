...
# which would indicate an invalid library object was returned for a valid library id
  ...
  exit 1
# Check that there's at least one linked library as we only take tumor only inputs
elif [[ "$(jq 'length' <<< "${libraries}")" -lt 1 ]]; then
  echo_stderr "Error: Less than 1 linked libraries found for the provided library IDs. At least one tumor library is required. Exiting."
  exit 1
else
