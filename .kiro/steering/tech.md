...
- **`@orcabus/platform-cdk-constructs`** — internal shared constructs library (pinned to `1.7.2`)
...
- **Amazon EventBridge** — event bus `OrcaBusMain`, source `orcabus.oncoanalyserwgtsrna`
- **AWS SSM Parameter Store** — configuration under `/orcabus/workflows/oncoanalyser-wgts-rna/`
- **AWS Schemas Registry** — JSON schema validation for draft events
- **Amazon ECS** — ECS tasks for BAM to FASTQ conversion

## Package Manager

**pnpm** (v11.8.0). Always use `pnpm`, never `npm` or `yarn`.
